...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
  ...
  cBox.setInputCloud (cloudFiltered);
...
//newRansacFumc for segmentation
template<typename PointT>
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
  ...
  for(pcl::PointIndices getIndices : cluster_indices){
    ...
    typename pcl::PointCloud<PointT>::Ptr obstClusters(new pcl::PointCloud<PointT>());
    
    for (int index : getIndices.indices){
    	obstClusters->points.push_back(cloud->points[index]);
    }
     obstClusters->width = obstClusters->size ();
     obstClusters->height = 1;
     obstClusters->is_dense = true;
                                       
  	clusters.push_back(obstClusters);
  }
    auto endTime = std::chrono::steady_clock::now();
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
...
template<typename PointT>
void ProcessPointClouds<PointT>::clusterHandler(typename pcl::PointCloud<PointT>::Ptr cloud, std::vector<bool>& processedPoints, int index, typename pcl::PointCloud<PointT>::Ptr cluster, KdTree* tree, float clusterTolerance)
{
    ...
    for (int id : proximity)
    {
        ...
        {
            clusterHandler(cloud, processedPoints, id, cluster, tree, clusterTolerance);
        }
    }
}
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::ClusteringEuclidean(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    ...
    for (int i = 0; i < cloud->points.size(); ++i)
    {
        ...
        clusterHandler(cloud, processedPoints, i, cluster, tree, clusterTolerance);
